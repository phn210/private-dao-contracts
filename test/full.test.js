...
describe("Test DAO Flows", () => {
  before(async () => {
    ...
    this.round2ContributionVerifier = await Round2ContributionVerifier.deploy();
    ...
    let Poseidon = await ethers.getContractFactory("Poseidon", this.founder);
  ...
  describe("Test DKG", async () => {
    it("Generate Funding Distributed Key", async () => {
      ...
      expect(await this.dkgContract.getDistributedKeyState(keyID)).to.be.equal(
        0
      );
  ...
  describe("Test FundManager", async () => {
    it("Success Flow", async () => {
      ...
      expect(await this.fundManager.getFundingRoundQueueLength()).to.be.equal(
        3
      );
      ...
      let [publicKeyX, publicKeyY] = await this.dkgContract.getPublicKey(keyID);
      ...
      for (let i = 0; i < this.voters.length; i++) {
        ...
        await this.fundManager
          ...
          .fund(fundingRoundID, fund.commitment, fund.Ri, fund.Mi, proof, {
      ...
      let tallyDataSubmissions = await this.dkgContract.getTallyDataSubmissions(
        requestID
      );
      ...
      for (let i = 0; i < VoterData.data1.fundingVector.length; i++) {
        ...
        for (let j = 0; j < fundingVector.length; j++) {
          result[j] +=
            VoterData.data1.votingPower[i] * BigInt(fundingVector[j]);
        }
      }
      // console.log(result);
      let { proof, publicSignals } = await snarkjs.groth16.fullProve(
      ...
      expect(await this.fundManager.getLastRoot()).to.be.eq(this.tree.root);

      // Withdraw fund to DAO
      for (let i = 0; i < result.length; i++) {
        let balanceBefore = await ethers.provider.getBalance(this.daos[i]);
        await this.fundManager.withdrawFund(fundingRoundID, this.daos[i]);
        let balanceAfter = await ethers.provider.getBalance(this.daos[i]);
        ...
        actions: [
          {
            ...
            data: ethers.utils.defaultAbiCoder.encode(["uint256"], [101]),
          },
        ],
        descriptionHash:
          "0x1d395c3cb1e6c1e9d9ad3eb571322666a8f5d45c99b67684a44c3a2b1ccda8fe",
